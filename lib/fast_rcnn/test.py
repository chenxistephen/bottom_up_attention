...
def _get_image_blob_448(im):
    ...
    for target_size in (TARGETSIZE,):
        im_scale = float(target_size) / float(im_size_min)
        # Prevent the biggest axis from being more than MAX_SIZE
        # if np.round(im_scale * im_size_max) > cfg.TEST.MAX_SIZE:
            # im_scale = float(cfg.TEST.MAX_SIZE) / float(im_size_max)
        im = cv2.resize(im_orig, (TARGETSIZE,TARGETSIZE))
        print im.shape
        im_scale_factors.append(im_scale)
        processed_ims.append(im)
...
def _get_blobs_448(im, rois):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {'data' : None, 'rois' : None}
    blobs['data'], im_scale_factors = _get_image_blob_448(im)
    if not cfg.TEST.HAS_RPN:
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    return blobs, im_scale_factors
    
    
def im_extract_features(net, im, boxes=None, force_boxes=False):
    ...
    """
    blobs, im_scales = _get_blobs_448(im, boxes)
    if force_boxes:
